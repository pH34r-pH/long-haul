...
def run(binary: str, model: str, output_dir: str, timeout_seconds: float = 900) -> ReferenceReport:
    out=Path(output_dir); report=ReferenceReport(environment={"platform":platform.platform()})
    report_path = out / "reference-report.json"
    def checkpoint() -> None: report.save(report_path)
    ...
    if validation.state is not ValidationState.SUPPORTED:
        report.record(3,LayerState.UNSUPPORTED if validation.state is ValidationState.UNSUPPORTED else LayerState.UNKNOWN,validation.rationale); checkpoint(); return report
    report.record(3,LayerState.PASS,validation.rationale); report.record(4,LayerState.PASS,"GGUF artifact present")
    request=ExecutionRequest(request_id=str(uuid4()),profile=profile,prompt="Return exactly: LONG_HAUL_OK",max_tokens=12,timeout_seconds=timeout_seconds)
    events.append(Event(event_type="request",source="reference",payload={"request_id":request.request_id,"state":"started"}))
    report.record(5,LayerState.UNKNOWN,"inference is in progress; a runner interruption leaves this checkpoint intact")
    checkpoint()
    direct=adapter.execute(request)
    if not direct.success:
        failure = LayerState.FAIL_RESOURCE if direct.error_class and direct.error_class.value == "FAIL_RESOURCE" else LayerState.FAIL_RUNTIME
        report.set_layer(5,failure,direct.error_detail or "inference failed"); checkpoint(); return report
    ...
    report.set_layer(5,LayerState.PASS,"normalized inference and model load succeeded")
    ...
    try:
        ...
        report.record(6,LayerState.FAIL_SYSTEM,f"benchmark persistence/query failed: {exc}"); checkpoint(); return report
    if not queried:
        report.record(6,LayerState.FAIL_SYSTEM,"benchmark persistence/query returned no observation"); checkpoint(); return report
    ...
    scheduled_request = ExecutionRequest(request_id=str(uuid4()),profile=profile,prompt="Say hello.",max_tokens=8,timeout_seconds=timeout_seconds)
    events.append(Event(event_type="decision",source="scheduler",payload={"plan_id":selected.plan.plan_id}))
    events.append(Event(event_type="request",source="reference",payload={"request_id":scheduled_request.request_id,"state":"started"}))
    report.record(8,LayerState.UNKNOWN,"scheduled execution is in progress; a runner interruption leaves this checkpoint intact")
    checkpoint()
    scheduled=adapter.execute(scheduled_request)
    if not scheduled.success:
        failure = LayerState.FAIL_RESOURCE if scheduled.error_class and scheduled.error_class.value == "FAIL_RESOURCE" else LayerState.FAIL_RUNTIME
        report.set_layer(8,failure,scheduled.error_detail or "scheduled execution failed")
        report.artifact["execution_result"]=scheduled.model_dump(mode="json")
        checkpoint()
        return report
    report.set_layer(8,LayerState.PASS,"scheduler-selected plan executed")
    events.append(Event(event_type="observation",source="runtime",payload={"success":True,"output":scheduled.output})); report.record(9,LayerState.PASS,"execution events persisted")
    materialize(events.events()); report.record(10,LayerState.PASS,"fresh event-log replay completed")
    report.artifact["execution_result"]=scheduled.model_dump(mode="json"); checkpoint(); return report
