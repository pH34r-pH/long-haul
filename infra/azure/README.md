...
Long Haul's Pi integration is pinned to upstream `@earendil-works/pi-coding-agent` 0.87.0 at commit `e40126f578ccc0a4a21f8d30cd8e96c8ecfe1722`. The protected `pi-upstream-qualification` reference operation prepares that exact source revision under `/opt/long-haul/pi-qualification` and verifies its package version without modifying system Python, llama.cpp, or the model. The operation downloads the official Node 22.19.0 Linux x64 archive into that disposable qualification directory, verifies its pinned SHA-256, installs Pi dependencies with `npm ci`, builds the pinned coding-agent workspace, and runs selected upstream deterministic tests for strict RPC JSONL framing, child-process failure handling, and native tool allowlisting. Node and Pi are never installed system-wide. Re-running the operation deletes and recreates only `/opt/long-haul/pi-qualification`; rollback is removal of that directory.
